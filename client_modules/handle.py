...
def upload_file(file_path, task_id, base_path, progress, KEY) -> None:
...
def handle_download_command(file_name, KEY):
    progress = Progress(
    ...
    with progress:
        with ThreadPoolExecutor() as pool:
            ...
            pool.submit(download_file, file_name, task_id, progress, KEY)

def handle_upload_command(path, KEY):
    file_paths = []
...
def handle_command():
    ...
    if command.startswith("key "):
        ...
        handle_upload_command(path, config.KEY)
